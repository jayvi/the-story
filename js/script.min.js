...
function slider() {
  $('#popSlide').slick({ infinite: !1 }),
    ($donorSlider = $('#donorSlider, #mDonorSlider').slick({ infinite: !1 })),
    $('#finSlider').slick({
      infinite: !1,
      slidesToShow: 3,
...
function preloadDesktop() {
  for (var e = 0; e < arguments.length; e++)
    (images2_al[e] = new Image()),
      (images2_al[e].src = preloadDesktop.arguments[e]);
}
var ww,
  wh,
...
function scrollMagic() {
  ...
  var o = new TimelineMax().add([
    TweenMax.fromTo(
      '.s2ImgAni1_1',
      4,
      { x: 0, scale: e },
      { scale: 1, x: -a, ease: Linear.easeNone }
    ),
    TweenMax.fromTo(
      '.s2ImgAni1_2',
      5,
  ...
  $('.s2Img1 img').each(function () {
    ...
    new ScrollMagic.Scene({
      ...
      .setClassToggle('.s2ImgAni1_2, .s2ImgAni2_2', 'leafMove')
      .addTo(controller);
}
function openPopup(e) {
  $('#popup' + e).fadeIn(300, function () {
    ...
    TweenMax.fromTo(
      ...
      0.8,
      { y: 15, autoAlpha: 0 },
      { y: 0, autoAlpha: 1, ease: Expo.easeOut, delay: 0.3 }
    );
}
function events() {
  $('.finRep').click(function () {
    ...
    $('.js-closebtn').click(function (e) {
      $('.popup').fadeOut(300, function () {
        ...
        $('html').removeClass('open-pop open-pop-style'),
        e.preventDefault();
    });
  var e = !0;
  $('#event-logo').click(function (a) {
    e
      ? (TweenMax.to('.sidebar', 0.8, { x: '0%', ease: Expo.easeOut }),
        $('.sidebar').addClass('menu-active'),
        (e = !1))
      : (TweenMax.to('.sidebar', 0.3, { x: '-100%' }),
    ...
    $(window).on('scroll', function () {
      $('.sidebar').hasClass('menu-active') &&
        (TweenMax.to('.sidebar', 0.3, { x: '-100%' }),
        $('.sidebar').removeClass('menu-active'),
        (e = !0));
    }),
    $(document).on('click', function (a) {
      $(a.target).closest('.sidebar, #event-logo').length ||
        (TweenMax.to('.sidebar', 0.3, { x: '-100%' }),
        $('.sidebar').removeClass('menu-active'),
        (e = !0));
    });
  var a = 50;
  $(window).width() <= 767 && (a = 30),
    $('.sidebar__inner a').click(function (e) {
    ...
    $('.js-readMore').click(function (e) {
      ...
      $('.readSlide' + a).slideToggle('easeOutSine'),
        $('.readSlide_inline' + a).toggle(),
        $(this).toggleClass('onSlide'),
        $('.readSlide' + a)
          .find('[data-scroll-reveal="enter from bottom"]:first')
          .addClass('revealText')
          .css({
...
(ww = $(window).width()),
  ...
  $(function () {
    slider(),
      ...
      defaultReplacer(),
      bubble(),
      new scrollReveal({ reset: !1 }),
      new SimpleBar(document.getElementById('simpleScroll')),
      new SimpleBar(document.getElementById('sideScroll'));
    //   $('.bubble').circleMagic()
  }),
  ...
  $(window).on('load', function () {
    loadDesk(),
      ...
      $('body').removeClass('onLoading').addClass('winLoaded'),
